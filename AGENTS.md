...
- 블로그 URL이 제공되면 제목, 날짜, `slug`, 태그는 자동 추출할 수 있다.
- 단, 요약문과 SEO description은 자동 생성하지 않는다.
- 사용자가 요약문을 제공하지 않은 경우, Markdown/HTML 생성 단계로 넘어가지 않는다.
- 요약문이 없으면 사용자에게 220~250자 요약문 제공을 요청하고 작업을 중단한다.
...
- 수집된 제목, 날짜, 본문과 사용자가 제공한 요약문을 기준으로 시공사례 HTML을 생성한다.
- 본문 수집이 성공하면 사용자가 본문 원고를 별도로 제공하지 않아도 된다.
- 본문 수집이 실패한 경우에만 사용자에게 원문 제공을 요청한다.
- 사용자가 별도로 제목, 날짜, `slug`, 태그를 제공하지 않아도 되지만, 요약문은 제공해야 한다.
...
- `go.py`가 수집한 제목, 날짜, 이미지와 사용자가 제공한 요약문이 실제 시공사례용으로 적합한지 확인한 뒤에만 다음 단계로 진행한다.
...
이 섹션은 새 게시물 작업에 우선 적용되는 보강 규칙이다. 기존 운영 규칙과 충돌하지 않도록, 새 게시물 생성 흐름에 한해 다음 규칙을 따른다.

### 10-1 기본 원칙

- 기존 운영 HTML과 URL을 임의로 삭제하거나 변경하지 않는다.
- 새 게시물은 폴더형 구조를 사용한다.
- Markdown 원본과 HTML 결과물, 이미지를 같은 게시물 폴더에 보관한다.
- Markdown은 원본, HTML은 생성 결과물이다.

### 10-2 새 게시물 구조

```text
cases/YYYY/case-YYYY-MM-DD-keyword-keyword/
├─ index.md
├─ index.html
├─ 01.jpg
├─ 02.jpg
...
- 새 게시물 이미지는 `assets/images/cases`에 저장하지 않는다.
- 새 게시물 이미지는 해당 게시물 폴더에 저장한다.
- Markdown 이미지 경로는 `./01.jpg` 같은 상대경로를 사용한다.
- 기존 게시물의 이미지 경로는 임의 변경하지 않는다.

### 10-4 요약문 규칙

- Codex가 SEO description이나 본문 요약을 임의 생성하지 않는다.
- 사용자가 제공한 요약문이 있을 때만 front matter `description`에 사용한다.
- 요약문이 없으면 작업을 중단하고 사용자 입력을 요청한다.
- 본문 앞부분을 잘라 description으로 쓰지 않는다.
- 추측 요약, 자동 감성 요약, 임의 SEO 설명문 생성을 금지한다.

### 10-5 HTML 생성

- `index.md`에서 `index.html`을 생성한다.
- 기존 오박사 홈페이지의 디자인, CTA, FAQ, JSON-LD, OG 태그 구조를 유지한다.
- CSS/JS 경로는 폴더형 URL에서도 깨지지 않도록 루트 기준 경로를 우선 사용한다.

### 10-6 목록 및 sitemap

- 새 게시물 생성 시 `cases.html`에 추가한다.
- 새 게시물 생성 시 `sitemap.xml`에 추가한다.
- 기존 URL은 임의 삭제하지 않는다.
- 중복 URL을 만들지 않는다.
...
- 예상하지 못한 변경 파일이 있으면 작업을 멈추고 보고한다.
- 대량 변경 전 변경 대상 파일 목록을 출력한다.
- 삭제 작업은 반드시 사용자 승인 후 진행한다.
- 테스트 후 `git diff --name-only`와 핵심 diff를 보고한다.
